...
트립 생성 시 기본 가방 두 개(`carry_on`, `checked`)가 자동 생성되며, 사용자 정의 가방은 `/trips/{trip_id}/bags` 엔드포인트로 추가합니다.
...
- 제약: `is_default=true` 금지, 동일 트립 내 `bag_type="carry_on"/"checked"`는 기본 가방만 존재.
- Response: `BagSummary`
